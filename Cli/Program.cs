...
class Program
{
    ...
    static async Task ExecuteMomo(MomoOptions options)
    {
        ...
        client.OnAllExpectationsMatch = async void (updatedFile) =>
        {
            await SaveChanges(client, options, updatedFile);
        };
        
        await client.MatchExpectations(CancellationToken.None);
    }

    public static async Task SaveChanges(IMomoClient sender, MomoOptions options, MomoExpectationFile file)
    {
        if (sender is AutoMomoClient autoMomoClient)
        {
            //Save json?
        ...
        {
            await MomoFileLoader.SaveAsync(file, options.InputFile);
